    ...
    create(){
       ...
       let start = this.add.image(
        ...
        .on('pointerdown', () => {
            // do event
            this.cameras.main.fade(500, 0,0,0);
            this.time.delayedCall(500, () => this.scene.start('Tutorial'));
        });
...
class Shop extends Phaser.Scene {
    ...
    create() {
        ...
        this.backSynth = new Tone.Synth().toDestination();
        this.notes = ["c3", "c#3", "d3", "d#3", "e3", "f3", "f#3", "g3", "g#3,", "a3", "a#3", "b3"]
        this.lengthMusic = ["2n", "4n", "8n", "16n"]
        this.timeMusic = 0
        this.playingMusic = false
        if(this.game.mute){
            this.backSynth.volume.value = -1000
        } else {
            this.backSynth.volume.value = 0
        }

        this.play_music()
        
        console.log(this.game.resources.blish)
        this.daytimer = 5000; // 2000
        ...
        this.input.on('pointerup', () => {
            if (this.dragging != null) {
                ...
                if (((temp.x > 1460) && (temp.x < 1810)) && ((temp.y > 725) && (temp.y < 975))) {
                    ...
                    if(this.game.resources.blish == 0)
                        this.blishPile.setAlpha(0)

                    if(this.game.resources.kukoi == 0)
                        this.kukoiPile.setAlpha(0)

                    if(this.game.resources.tazoi == 0)
                        this.tazoiPile.setAlpha(0)
        ...
        .setInteractive()
        .on('pointerdown', () => {
            if (this.blishPile.active && this.game.resources.blish > 0) {
        ...
        .setInteractive()
        .on('pointerdown', () => {
            if (this.tazoiPile.active && this.game.resources.tazoi > 0) {
        ...
        .setInteractive()
        .on('pointerdown', () => {
            if (this.kukoiPile.active && this.game.resources.kukoi > 0) {
    ...
    play_music() {
        if (!this.playing) {
            ...
            this.backSynth.triggerAttackRelease(this.notes[rng], this.lengthMusic[rand], this.timeMusic);
            this.timeMusic += 2 / (1 + rand)
...
class Earnings extends Phaser.Scene {
    ...
    create() {
        ...
        .on('pointerdown', () => {
            ...
            if(this.game.scoreboard.money >= 7){
                this.game.resources.tazoi += 1
                this.game.scoreboard.money -= 7
                this.synth.triggerAttackRelease("c4", "0.3");
                //this.updateText();
                this.createNumber(1250,380, this.fundsNumber, this.game.scoreboard.money);
                this.createNumber(1360,510, this.tazoiNumber, this.game.resources.tazoi);
...
class Credits extends Phaser.Scene {
    ...
    create(){
        ...
        this.add.text(200,300, data["credits"][0]).setTint(0xaa00aa).setFontSize(50);
...
class Settings extends Phaser.Scene {
    ...
    create(){
        ...
        this.muteOn = this.add.text(700,450, "music: on").setTint(0xaa00aa).setFontSize(75).setInteractive();

        this.muteOn.on('pointerdown', () => {
            this.muteOn.setAlpha(0)
            this.muteOff.setAlpha(1)
            this.game.mute = true
        });

        this.muteOff = this.add.text(700,450, "music: muted").setTint(0xaa00aa).setFontSize(75).setInteractive();

        this.muteOff.on('pointerdown', () => {
            this.muteOff.setAlpha(0)
            this.muteOn.setAlpha(1)
            this.game.mute = false
        });
        ...
        this.backButton = this.add.text(200,900, "Go back").setTint(0xaa00aa).setFontSize(75).setInteractive();
...
const game = new Phaser.Game({
    ...
    scene: [Start, intro, Menu, Tutorial, Shop, Earnings, Credits, Settings],
    title: "Sushi Game"
});

game.scoreboard = {'money' : 10, 'correct' : 0, 'wrong' : 0, 'timeout' : 0};
game.day = 1;
game.resources = {"blish" : 7, "kukoi" : 7, "tazoi" : 7, "seaweedPiece" : Infinity, "riceClump" : Infinity};
game.mute = false;